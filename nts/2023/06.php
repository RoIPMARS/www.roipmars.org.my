<!DOCTYPE html>
<html lang='ms-MY' data-bs-theme='auto'>

  <head>
    <title>Peralihan Pelayan roipmars.com</title>
  </head>

  <body>
    <?php include __DIR__.'/../../ntsheader.php' ?>
    <div class='container my-2'>
      <h1 class='text-center'>Peralihan Pelayan roipmars.com</h1>
      <table class='table table-sm table-borderless fs-5 text-center d-flex justify-content-end'>
        <tbody>
          <tr><td>Rujukan Notis</td><td>NtsMARS2023/06</td></tr>
          <tr><td>Tarikh Notis</td><td>10 Mei 2023, 0230 MYT</td></tr>
        </tbody>
      </table>
      <p class='lh-sm'>Pengguna yang dihormati,</p>
      <p>Adalah dimaklumkan bahawa semua maklumat dan <i>data</i> yang kini berada di pelayan roipmars.com &lpar;IPv4: 101.xx.xx.75&rpar; akan beralih ke pelayan lain dengan IP yang baru pada ketetapan berikut;</p>
      <table class='table table-sm table-bordered'>
        <tbody>
          <tr><th>Tarikh</th><td>02 Jun 2023 - 05 Jun 2023</td></tr>
          <tr><th>Masa Gangguan</th><td>12 tengah malam waktu Malaysia</td></tr>
          <tr><th>Tempoh Gangguan</th><td>5 Jam</td></tr>
          <tr><th>Perkhidmatan Terjejas</th><td><ol type='i'><li>Laman sesawang roipmars.com</li><li>TeamSpeak<ul><li><a href='ts3server://smp.roipmars.com'>smp.roipmars.com</a> &lpar;semua maklumat akan diset semula&rpar;</li></ul></li></ol></td></tr>
        </tbody>
      </table>
      <p>Dinasihatkan supaya semua pengguna <i>domain</i> dan <i>subdomain</i> roipmars.com untuk menggunakan <i>domain</i> sebagai sambungan utama bagi mengelakkan sebarang masalah dimasa akan datang. <strong class='text-danger-emphasis'>Penggunaan <i>IP</i> untuk sambungan utama tidak digalakkan.</strong></p>
      <p>Pelayan dan sambungan akan mengalami gangguan semasa tempoh peralihan. Dinasihatkan supaya tidak melakukan sebarang aktiviti semasa tempoh peralihan.</p>
      <p>p/s: <i>domain</i> dan pelayan roipmars.com adalah milik sepenuhya syarikat swasta &lpar;bukan pemilikan pertubuhan&rpar;.</p>
      <p class='text-center'>Diharap pihak tuan dapat ambil maklum berkenaan perkara ini. Persefahaman dan kerjasama anda amat kami hargai. Jutaan kemaafan kami pohon dari Pihak Pengurusan Kumpulan RoIPMARS kepada anda semua.</p>
      <p class='text-center'>Sekiranya anda memerlukan bantuan, sila emel kepada: <a href='contact'>member@roipmars.org.my</a></p>
      <p class='fs-4 text-end mt-2 mb-0 ft-signature'>Pentadbir Rangkaian</p>
      <p class='text-end mt-0 mb-2 ft-signature1'>Pengurusan Rangkaian Kumpulan RoIPMARS</p>
    </div>
    <?php include __DIR__.'/../../footermin.php' ?>
  </body>

</html>