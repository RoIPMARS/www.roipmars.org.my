		...
		$('#takwim').delegate('tbody tr td:last-child', 'click', function () {
			...
			var netReportTable = $(`#${reportID}`).DataTable({
				...
				initComplete: function (settings, json) {
					...
					netRepModGraph.id = `net-${source}-modgraph`
			...
			$(`#${reportID}`).delegate('tbody tr td:nth-child(1)', 'click', async function () {
				...
				async function generateCert(date, activity, ncs, caller, modes, utctime) {
					...
					msgInfo.show()
					try {
						let respCALL = await fetch(`${hookAPI.URL}/csnames?callsign=${caller}`)
						if (respCALL) {
					...
					if (ncs != caller) {
						...
						try {
							let respNCS = await fetch(`${hookAPI.URL}/csnames?callsign=${ncs}`)
							if (respNCS) {
					...
					msgInfo.show()
					try {
						let respCtc = await fetch(`${hookAPI.URL}/callctc?callsign=${caller}`, {
							method: 'GET',
							headers: { 'content-type': 'application/json' },
						})
						if (respCtc) {
							...
							if (checkMail > 0) {
								...
								await fetch(`${mailAPI.BaseURL}/smtp/email`, {
									...
									body: JSON.stringify({
										...
										htmlContent: `<html><body><p>Hi, thank you for using our services. Here is your requested certificate;</p><table><tr><td>CallSign</td><td>${caller}</td></tr><tr><td>Time</td><td>${dtl.toUTCString()}</td></tr></table><p>You have requested a certificate from our records via ${location} on ${new Date().toString()}.</p><p>Please keep it in a safe place.For any questions, please contact one of our administrators.<br><br>Sincerely,<br>Records Division, RoIPMARS</p></body></html>`,
